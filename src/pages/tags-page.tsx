...
const colorChoices = ["#f43f5e", "#fb7185", "#ec4899", "#d946ef", "#f97316", "#f59e0b", "#84cc16", "#10b981"];
      ...
      <section className="space-y-3">
        <div className="flex h-12 items-center">
          <div className="flex min-w-0 flex-1 items-center gap-2 rounded-[24px] border border-rose-100 bg-white px-3 py-2.5">
            ...
            />
            {activeTagId ? (
              <button type="button" onClick={() => setActiveTagId(null)} className="shrink-0 text-xs font-semibold text-rose-600">
                {t("viewAll")}
              </button>
            ) : null}
            <button
              ...
              className="inline-flex h-8 shrink-0 items-center rounded-xl bg-rose-600 px-3 text-xs font-semibold text-white shadow-lg shadow-rose-900/15"
            >
        ...
        <div className="-ml-14 w-[calc(100%+3.5rem)] space-y-3 pt-2">
          {isCreateOpen ? (
            <form className="space-y-3 rounded-[32px] border border-rose-100 bg-rose-50/70 p-5" onSubmit={handleCreateTag}>
              <input
                value={name}
                onChange={(event) => setName(event.target.value)}
                placeholder={t("tagName")}
                className="w-full rounded-2xl border border-rose-100 bg-white px-4 py-3 text-sm outline-none focus:border-rose-400"
              />
              <div className="flex flex-wrap gap-2">
                {colorChoices.map((choice) => (
                  <button
                    ...
                    style={{ backgroundColor: choice, borderColor: color === choice ? "#e11d48" : "#ffffff" }}
                  />
                ))}
              </div>
              <button type="submit" className="w-full rounded-2xl bg-rose-600 px-4 py-3 text-sm font-semibold text-white transition hover:bg-rose-700">
                {t("createTag")}
              </button>
            </form>
          ) : null}
