...
import { Camera, Search, Store, Trash2 } from "lucide-react";
...
export default function MenuPage() {
  ...
  const filteredItems = useMemo(() => {
    ...
    return sortedItems.filter((item) => item.restaurantName.toLowerCase().includes(normalizedQuery));
      ...
      <section className="-ml-14 w-[calc(100%+3.5rem)] space-y-3 pt-0.5">
        ...
        {filteredItems.length === 0 ? (
          <div className="rounded-[28px] border border-dashed border-slate-300 bg-white px-4 py-8 text-center text-sm text-slate-500">
            {normalizedQuery ? t("searchMenu") : t("noMenuItems")}
          </div>
        ) : (
